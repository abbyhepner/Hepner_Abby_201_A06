...
[https://github.com/abbyhepner/Hepner_Abby_201_A06](https://github.com/abbyhepner/Hepner_Abby_201_A06)
...
```{r}
# using indexing
points_LJ1 <- player_info[[2]][[4]]

# using names
points_LJ2 <- player_info[["LJ"]][["points"]]
...
# calculate points_per_game for each player
for (player in names(player_info)) {
...
```{r}
# round each val to two decimal places
rounded_points_per_game <- sapply(points_per_game, round, 2)
...
```{r}
round_based_on_digits <- function(x) {
  if (x < 100) {
    return(round(x, -1))  # round to nearest ten
  } else if (x < 1000) {
    return(round(x, -2))  # round to nearest hundred
  } else {
    return(round(x, -3))  # round to nearest thousand
  }
}

# apply function to each element in matrix
rounded_mat_rand <- apply(mat_rand, 1:2, round_based_on_digits)

# print both original and rounded matrices (just to check)
mat_rand
rounded_mat_rand
```
...
```{r}
average_temps <- colMeans(max_temps[,-1])
...
```{r}
# calculate median annual temp for Colorado
median_CO_temps <- apply(CO_temps[,-1], 1, median)

# find year with highest and lowest median temp
max_median_temp <- max(median_CO_temps)
min_median_temp <- min(median_CO_temps)
max_year <- CO_temps$year[which.max(median_CO_temps)]
min_year <- CO_temps$year[which.min(median_CO_temps)]
...
```{r}
# recursive function to sum digits of a num
...
# test function with 935068
result <- sum_digits(935068)
